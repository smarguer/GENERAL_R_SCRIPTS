...
#str(paste("chr",chr,sep=""))
MyplotAlong(highlight1=highlight1,highlight2=highlight2, highlight3=highlight3, highlight4=highlight4, y = as.matrix(n), chr = chr, coord = range * 1000, gff = mygff, probeAnno=probeAnno, what=c("dots"),sepPlots=sep, featureExclude = c("LTR","misc_RNA","new","new_antisense", "gap","3'UTR", "5'UTR", "CDS_motif", "gap", "mRNA", "polyA_site", "promoter","rep_origin", "repeat_unit","polyA_signal","BLASTN_HIT", "trans","utr_intron"))
...
#str(paste("chr",chr,sep=""))
MyplotAlong(highlight1=highlight1,highlight2=highlight2, highlight3=highlight3, highlight4=highlight4, y = as.matrix(n), chr = chr, coord = range * 1000, ylim = scale, gff = mygff, probeAnno=probeAnno, what=c("heatmap"),sepPlots=sep, featureExclude = c("LTR","misc_RNA","new","new_antisense", "gap","3'UTR", "5'UTR", "CDS_motif", "gap", "mRNA", "polyA_site", "promoter","rep_origin", "repeat_unit","polyA_signal","BLASTN_HIT", "trans","utr_intron"))
...
function (segObj, y, probeAnno, gff, isDirectHybe = FALSE, what = c("dots"), 
    ...
    main, colHeatmap = colorRamp(brewer.pal(9, "YlGnBu")), colExtras = colorRamp(brewer.pal(9, 
        "Reds")), sepPlots = FALSE, reOrder = TRUE,mixScale=FALSE,scaleU=NULL,scaleD=NULL, ...) 
{
...
###################################
                ...
                for (j in seq(1:n)) {
                  ...
                  vpr = which(names(VP) == sprintf(paste("expr", 
                    j, "%s", sep = ""), strand))
                  #ylim = ylim[j,] in order to permit scale adjustment# 
                    MyplotSegmentationDots(datj, xlim = coord, ylim = ylim[j,],
