plot.geneOrientation=function(output=out,title="Default")
{
plot(row.names(output),-log10(output[,1]),ylim=c(0,10),type="b",pch=20,lwd=3, xlab="ranked bin", ylab="-log10 binomial p.value", main=title)
lines(row.names(output),-log10(output[,2]),type="b",col="blue",pch=20,lwd=3)
lines(row.names(output),-log10(output[,3]),type="b",col="green",pch=20,lwd=3)
lines(row.names(output),-log10(output[,4]),type="b",col="pink",pch=20,lwd=3)
lines(row.names(output),-log10(output[,5]),type="b",col="orange",pch=20,lwd=3)
lines(row.names(output),-log10(output[,6]),type="b",col="red",pch=20,lwd=3)
lines(row.names(output),-log10(output[,7]),type="b",col="purple",pch=20,lwd=3)
abline(h=c(1.3,3))
legend(x="topleft",legend=colnames(output),fill=c("black","blue","green","pink","orange","red","purple"))
}



sliding.geneOrientation=function(data1=test,data2=RPKM,totest="il_wt1",li=c(1:nrow(data2)),sreal=T,win=100, step=50, alt="greater",adjust=T)
{
...
plot.geneOrientation(output=outG,title=paste(totest,"greater",sep=" "))
plot.geneOrientation(output=outL,title=paste(totest,"less",sep=" "))
