...
####################################################################################
viewSEQ.sep<-function(c1=chr1, c2=chr2, c3=chr3, range=c(1478,1490),scale=c(-4,20), highlights=c(1487000,1489000,1481000,1485000), gff=mygff,chr=1, sep=T,...){
grid.newpage()
if (chr==1){n=c1}
else if (chr==2){n=c2}
else if (chr==3){n=c3}
...
MyplotAlong(highlight1=highlight1,highlight2=highlight2, highlight3=highlight3, highlight4=highlight4, y = as.matrix(n), chr = chr, coord = range * 1000, gff = mygff, probeAnno=probeAnno, what=c("dots"),sepPlots=sep, featureExclude = c("LTR",
"misc_RNA","new","new_antisense", "gap","3'UTR", "5'UTR", "CDS_motif", "gap", "mRNA", "polyA_site", "promoter",
"rep_origin", "repeat_unit","polyA_signal","BLASTN_HIT"))

gc()
}

####################################################################################
viewGEN.sep<-function(gene='SPAC1250.05',c1=chr1, c2=chr2, c3=chr3,flank=2,scale=c(-4,20), gff=mygff, sep=T,...){
grid.newpage()
t=getc(gene)
if(nrow(t)==0)
{
stop ("Sorry, unknown gene name.")
}
chr=as.numeric(t[2])
if (chr==1){n=c1}
else if (chr==2){n=c2}
...
start=floor(as.numeric(t[3])/1000)-flank
end=ceiling(as.numeric(t[4])/1000)+flank
range=c(start,end)

highlight1=list()
highlight1$coord=as.numeric(t[3])
highlight1$strand="+"

highlight2=list()
highlight2$coord=as.numeric(t[4])
highlight2$strand="+"
...
#str(paste("chr",chr,sep=""))
MyplotAlong(highlight1=highlight1, highlight2=highlight2, y = as.matrix(n), chr = chr, coord = range * 1000, gff = mygff, probeAnno=probeAnno, what=c("dots"),sepPlots=sep, featureExclude = c("LTR",
"misc_RNA","new","new_antisense", "gap","3'UTR", "5'UTR", "CDS_motif", "gap", "mRNA", "polyA_site", "promoter",
"rep_origin", "repeat_unit","polyA_signal","BLASTN_HIT"))
...
if (all==T)
{
out=mygff[which((mygff[,"Name"]==gene)),c("Name","chr","feature","start","end","strand","attributes")]
}
else if (all==F)
{
out=mygff[which((mygff[,"feature"]=="gene") & (mygff[,"Name"]==gene)),c("Name","chr","start","end","strand","attributes")]
}
return(out)
}
